...
import { ROLES, CASE_STATUS, EXAMPLE_CASE_NUMBER, EXAMPLE_TASK_NUMBER } from "./GLOBAL_PARAMETERS";
...
export const canViewCase = (currentUser: IMe, caseData: ICase): boolean => {
  ...
  if (caseData.case_number === EXAMPLE_CASE_NUMBER) return true;
...
export const canViewTask = (currentUser: IMe, task: ITask): boolean => {
  ...
  if (task.taskNumber === EXAMPLE_TASK_NUMBER) return true;
